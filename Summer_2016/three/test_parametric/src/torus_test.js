...
var torusRad = bobRad*2; var torusTube = torusRad/4;
...
function init() {
    ...
    // Torus Map
    torus = new THREE.Mesh(new THREE.TorusGeometry( torusRad, torusTube, 16, 100 ), new THREE.MeshBasicMaterial( { color: 0xffff00, transparent: true, opacity: 0.7 }));
    log(torus);
    torus.position.set(bobRad*7, 0, bobRad*7);
    scene.add(torus);
    /*var testSphere = new THREE.Mesh(new THREE.SphereGeometry( bobRad/5, 32, 32 ), new THREE.MeshBasicMaterial( {color: 0x00ff00} ));
    testSphere.position.x = torus.position.x;
    testSphere.position.y = torus.position.y;
    testSphere.position.z = torus.position.z;
    ...
    var pos = torusCoordinates(BobMap.uvPos.x,BobMap.uvPos.y,torusRad,torusTube,torus.position);
    log('torus position:');
    log(pos);
...
function render() {
    ...
    if(delta <1){
        ...
        log(BobMap.uvPos);
        currPos = torusCoordinates(BobMap.uvPos.x,BobMap.uvPos.y,torusRad,torusTube,torus.position);
...
function torusCoordinates(u, v, R, d, center){
    var tx, ty, tz;
    //R = R + d/2;
    /*
    tx = center.x+(R+d*Math.cos(2*Math.PI*u)/2)*Math.cos(2*Math.PI*v);
    ty = center.y+(R+d*Math.cos(2*Math.PI*u)/2)*Math.sin(2*Math.PI*v);
    tz = center.z+d*Math.sin(2*Math.PI*u)/2;*/
    tx = center.x+(R+d*Math.cos(2*Math.PI*u))*Math.cos(2*Math.PI*v);
    ty = center.y+(R+d*Math.cos(2*Math.PI*u))*Math.sin(2*Math.PI*v);
    tz = center.z+d*Math.sin(2*Math.PI*u);
    return V3(tx, ty, tz);
}
